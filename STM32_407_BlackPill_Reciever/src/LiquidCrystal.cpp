...
//   pinMode(_rs_pin, OUTPUT);
  gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT,GPIO_PUPD_NONE, _rs_pin);
  // we can save 1 pin by not using RW. Indicate by passing 255 instead of pin#
  if (_rw_pin != 255) { 
    // pinMode(_rw_pin, OUTPUT);
  gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT,GPIO_PUPD_NONE, _rw_pin);
	
  }
//   pinMode(_enable_pin, OUTPUT);
  gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT,GPIO_PUPD_NONE, _enable_pin);
  ...
  gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT,GPIO_PUPD_NONE, _data_pins[i]);
...
void LiquidCrystal::pulseEnable(void) {
  ...
  delay_us(10);    // enable pulse must be >450 ns
  pinWrite(GPIOB,_enable_pin, 0);
